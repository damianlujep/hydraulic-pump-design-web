...
- `IprChart` — IPR curve (Vogel or Fetkovich, same shape, legend swaps to name whichever ran) with a static reservoir pressure line and a marked bubble point (Pb); when a design oil rate was requested, an extra green "Punto de diseño" marker (dashed guides + dot + label, sharing a `Marker` sub-component with the Pb marker) plots the design point, and the stat row below gains the Fetkovich exponent (`n`) and the design point's Pwf/Q/Qo values.
